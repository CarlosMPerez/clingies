using System;
using Gtk;
using appIndiNative = Clingies.Utils.AppIndicatorNative;
...
public sealed class AppIndicator : IDisposable
{
    ...
    public AppIndicator(string id, string iconName, Enums.AppIndicatorCategory category)
    {
    ...
    public void SetStatus(Enums.AppIndicatorStatus status) => appIndiNative.app_indicator_set_status(_handle, status);
